...
from pipeline.ingestion.csv_ingestion import load_and_preprocess
from pipeline.common.visualization import plot_forecast, plot_overview
...
class ServerMetricsForecaster:
    ...
    def predict(
        ...
        for metric in METRICS:
            ...
            if save_charts:
                plot_forecast(
                    ...
                    save=True,
                    output_dir=FORECASTS_NP_CSV_DIR,
                )
        ...
        if save_charts:
            ...
            print(f"\n✅ Charts saved to: {FORECASTS_NP_CSV_DIR}/")
    ...
    def _model_path(host_alias: str, metric: str, n_lags: int) -> Path:
        seg_path = NP_CSV_MODELS_DIR / f"{host_alias}_{metric}_lags{n_lags}.np"
        legacy_path = MODELS_DIR / f"{host_alias}_{metric}_lags{n_lags}.np"
        if seg_path.exists():
            return seg_path
    ...
    def _save_model(
        ...
        NP_CSV_MODELS_DIR.mkdir(parents=True, exist_ok=True)
        path = NP_CSV_MODELS_DIR / f"{host_alias}_{metric}_lags{n_lags}.np"
